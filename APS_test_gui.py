...
# main file
# TO TEST: change keithley_gatecurrent to HPPT
# TO TEST: implement AUX PSU control (voltage, current, enable/disable)
...
class MainWindow(ManagedDockWindow):
    ...
    def __init__(self, startup_config=None):
        ...
        # Configure based on selected procedure
        if procedure_class == HighPowerPulseTest:
            inputs_list = ['aps_port', 'keithley_resource', 'measurement_voltage', 'wait_for_completion', 'timeout']
            displays = ['aps_port', 'keithley_resource', 'measurement_voltage', 'wait_for_completion', 'timeout']
            x_axis = ['Timestamp']
            y_axis = ['Current (A)', 'Burst Count']
        else:  # RandomProcedure
            inputs_list = ['iterations', 'delay']
        ...
        try:
            ...
            log.info(f'Current directory after application: {getattr(self, "directory", "NOT SET")}')
        except Exception:
    ...
    def _do_populate_parameters(self, connection_params):
        ...
        try:
            ...
            # Keithley parameters
            keithley_params = connection_params.get('keithley_2470', {})
            if keithley_params:
                ...
                if hasattr(self.inputs, 'measurement_voltage'):
                    voltage = keithley_params.get('voltage', 20.0)
                    self.inputs.measurement_voltage.setValue(voltage)
